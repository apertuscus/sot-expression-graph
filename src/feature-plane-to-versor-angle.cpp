...
#include "feature-plane-to-versor-angle.h"
#include "helper.h"
...
DYNAMICGRAPH_FACTORY_ENTITY_PLUGIN(FeaturePlaneToVersorAngle,"FeaturePlaneToVersorAngle");

/* --------------------------------------------------------------------- */
/* --- CLASS ----------------------------------------------------------- */
/* --------------------------------------------------------------------- */

FeaturePlaneToVersorAngle::
FeaturePlaneToVersorAngle( const string& name )
...
, referenceSIN( NULL,"FeatureAnglesBtwPlanes("+name+")::input(vector)::reference" )
{
  ...
  signalRegistration( p1_SIN  <<norm1_SIN<<versor2_SIN << referenceSIN);
  ...
  Soutput_ = geometric_primitive::angle_btw_plane_and_versor(plane,versor);
...
unsigned int& FeaturePlaneToVersorAngle::
...
void FeaturePlaneToVersorAngle::updateInputValues(KDL::Expression<double>::Ptr Soutput, int time)
{
...
ml::Matrix& FeaturePlaneToVersorAngle::
 ...
 */
ml::Vector&
FeaturePlaneToVersorAngle::computeError( ml::Vector& res,int time )
{
  sotDEBUGIN(15);
